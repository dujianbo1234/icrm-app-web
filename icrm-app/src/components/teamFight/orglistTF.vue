		...
		<van-popup v-model:show="show" position="bottom" :style="{ height: '55%' }" teleport="#app" :overlay="overlay">
			...
			<div class="listBox">
				...
				<div class="orgItem" v-for="(orgItem,i) in orgList" :key="'orgItem'+i"
					:style="{'padding-left':((orgItem.level-startLevel)*0.26)+0.12+'rem'}"
					:class="orgItem.value==chooseOrg.value?'orgItem_a':''" @click="checkOrg(orgItem)">
					<van-icon :name="orgItem.open?'arrow-down':'arrow'"
						:color="orgItem.level<3?'#0088FF':orgItem.value==chooseOrg.value?'#DFDFDF':'#FFFFFF'"
...
<script>
	...
	export default {
		...
		methods: {
			showPopup(searchOrg,type) {
				this.chooseOrg = searchOrg;
				if(searchOrg.value){
					this.show = true;
				}else{
					this.postMsg.branchOrgType = type;
					this.orgList = [];
					this.show = true;
					this.getOrgList({
						text: "",
			...
			getOrgList(item) {
				if (item.open) {
					...
					this.postMsg.orgType = (item.level + this.startLevel).toString();
					this.postMsg.orgNo = item.value;
					getOrgTree(this.postMsg, (res) => {
							...
							if (res.data.length) {
								var addList = res.data.map((resItem) => {
									return {
										...
										value: resItem.orgNo,
										level: item.level + this.startLevel,
										open: false
									}
								})
							} else {
