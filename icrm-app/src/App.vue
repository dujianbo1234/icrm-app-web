<template ref="appBox">
  ...
  <div class="shuiyin" v-if="showSY">
    <span style="margin-right: 1rem" v-for="i of 100" :key="i">{{ $store.state.userMsg.empname }} {{ $store.state.userMsg.empno }}{{ $store.state.userMsg.loginTime }}</span>
  </div>
  <van-popup v-model:show="showSecret" round :close-on-click-overlay="false" :overlay-style="{ 'background-color': '#888888' }">
    <div class="secretBox">
      ...
      <div class="secretBtn">
        ...
        <div
          :class="checked ? 'secretBtn_confirm_a' : 'secretBtn_confirm'"
          @click="confirmSecret"
        >
          确认
        </div>
