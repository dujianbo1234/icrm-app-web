	...
	<div class="home">
			...
			<div class="topMsg">
				...
				<div class="topMsgBar">
					...
					<div class="msgBarItem" @click="mustDo">
						<i class="msg-icon icon-todo"></i>必办<span v-if="mustDoNum > 0"
		...
		<div class="mainContent">
			<div class="selectTimebar">
				<org-list :type="0" showText="请选择机构" v-if="$store.state.userMsg.roleId!='00000004'"
					@activeOrg="khgmActiveOrg" />
				<div v-else></div>
				<div class="currentDate">当前数据时间：{{ showDate }}</div>
			</div>
			<div class="contentItem week" :class="showWeekDetial ? 'open' : 'close'"
				v-if="$store.state.userMsg.roleId != '00000006' && $store.state.userMsg.roleId != '00000010'">
				...
				<div class="weekList" ref="weekList">
					<div class="weekItem" v-for="(item, i) in weekList" :key="'week' + i" @click="showWeek(item)">
						<div class="dateTitle">
							...
							<div :class="item.toDay ? 'today' : item.disabe ? 'future' : 'weekDate'">{{ item.showDate }}
							</div>
						</div>
					</div>
				</div>
				<div class="arrow" @click="showWeekDetial = !showWeekDetial">
					<!-- 分割线 -->
					<van-divider>
						<van-icon :name="showWeekDetial ? 'arrow-up' : 'arrow-down'" size="0.22rem" color="#ccc" />
					</van-divider>
				</div>
				...
				<div class="khgmBox">
					<div class="khgmItemBox" v-for="(item, i) in showData" :key="'khgm' + i">
						<div class="khgmMainTitle">
							...
							<van-icon style="margin-left: 0.058rem" @click="clickDalong(item.title, item.text)"
								:name="require('../../assets/image/index_question.png')" size="0.133rem" />
						</div>
						...
						<div class="khgmTitle">
							...
							<div class="khgmSubValue">
								<p>{{ item.value1 || "0" }}</p>
								<van-icon v-if="   !item.value1 || item.value1 == '0' || item.value1 == '0.00' "
									:name="require('../../assets/image/index_main_numLine.png')" size="0.16rem" />
								<van-icon v-else-if="item.value1[0] == '-'"
									:name="require('../../assets/image/index_arrow_dowm.png')" size="0.16rem" />
								<van-icon v-else :name="require('../../assets/image/index_arrow_top.png')"
									size="0.16rem" />
							</div>
						</div>
						<div class="khgmTitle">
							...
							<div class="khgmSubValue">
								<p>{{ item.value2 || "0" }}</p>
								<van-icon v-if="!item.value2 || item.value2 == '0' || item.value2 == '0.00'"
									:name="require('../../assets/image/index_main_numLine.png')" size="0.16rem" />
								<van-icon v-else-if="item.value2[0] == '-'"
									:name="require('../../assets/image/index_arrow_dowm.png')" size="0.16rem" />
								<van-icon v-else :name="require('../../assets/image/index_arrow_top.png')"
									size="0.16rem" />
							</div>
						...
						<div class="khgmTitle">
							...
							<div class="khgmSubValue">
								<p>{{ item.value3 || "0" }}</p>
								<van-icon v-if="!item.value3 || item.value3 == '0' || item.value3 == '0.00'"
									:name="require('../../assets/image/index_main_numLine.png')" size="0.16rem" />
								<van-icon v-else-if="item.value3[0] == '-'"
									:name="require('../../assets/image/index_arrow_dowm.png')" size="0.16rem" />
								<van-icon v-else :name="require('../../assets/image/index_arrow_top.png')"
									size="0.16rem" />
							</div>
			...
			<div class="contentItem" style="margin-top: 0.12rem">
				<TitleCard :title="['AUM余额','AUM日均']" :arr="peCstAum" @change="peCstAumChange"
					@clickDalong="clickDalong(' ',['AUM考核口径不包括储蓄存款'])"></TitleCard>
			</div>
			<!-- 分割线 -->
			<div class="dividers"></div>
			<!-- AUM余额分布图 -->
			<div class="contentItem">
				...
				<!-- 列表 -->
				<div v-if="aumFlag == 0">
					<Table :listLabel="[listLabel,listLabels][listType]" :listData="[listData,listDatas][listType]"
						:listType="listType" :lableArr="[['a','b','c', 'd'],['a','b','c']][listType]"></Table>
				</div>
			...
			<!-- 贷款余额(万元) -->
			<div class="contentItem" style="margin-top: 0.12rem">
				<TitleCard :title="['贷款余额','贷款日均']" :arr="peCstLoan" @change="peCstLoanChange"
					@clickDalong="clickDalong(' ',['零售贷款余额'])"></TitleCard>
			</div>
			<!-- 分割线 -->
			<div class="dividers"></div>
			<!-- 贷款余额-增长趋势 -->
					...
					if (res.data && res.data.records && res.data.records.length) {
						...
						this.listData = [{
							...
							},
							{
								...
								d: dataObj.entrstBalToBegng
							},
							{
								name: '合计',
								a: dataObj.aumBal,
								b: dataObj.aumBalToYstd,
								c: dataObj.aumBalToLastMonth,
								d: dataObj.aumBalToBegng
							},
						]
						this.listDatas = [{
								name: '活期存款',
							...
							{
								name: '合计',
								a: dataObj.aumYearAvg,
								b: dataObj.aumYearAvgToLm,
								c: dataObj.aumYearAvgToYy
							},
						]
							...
							switch (item.title) {
								case "零售客户数":
									[item.value0, item.value1, item.value2, item.value3] = [
										...
										return item <= 0 || index == 0 ? formatNums(item) :
											`+${formatNums(item)}`;
									});
									break;
								case "财富客户数":
