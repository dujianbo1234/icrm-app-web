<template>
	<div class="home">
		<nav-bar v-if="$store.state.userMsg.roleId=='00000004'" title="重要潜客列表" type="2" leftIcon />
		<nav-bar v-else title="重要潜客列表" type="2" leftIcon :rightText="chooseOrg.text" rightColor="#026DFF"
		...
		<div class="fixedPlace">
			...
			<van-search v-else v-model="searchValue" shape="round" show-action placeholder="请输入客户名称/归属客户经理工号"
		...
		<van-list v-model:loading="loading" :finished="finished" finished-text="没有更多了" @load="onLoad">
			<van-checkbox-group v-model="checked" ref="checkboxGroup">
				<div v-for="(msgItem,i) in msgList" :key="'msgItem'+i" class="msgCardOutBox"
					...
					<van-swipe-cell :disabled="openPLFP">
						<template #default>
						...
						<template #right>
							<div class="delBtn" @click="delCust(msgItem.custNo)">删除</div>
						</template>
					</van-swipe-cell>
				</div>
			</van-checkbox-group>
...
<style scoped>
	* {
	...
	.delBtn {
		...
		background: url(../../assets/image/cust_zyqk_delete.png) no-repeat center top;
